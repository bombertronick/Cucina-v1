...
window.togglePeakOverride = async () => {
    ...
    await lazzaro_saveState();
    window.renderApp();
    if(window.showToast) window.showToast(State.peakOverride ? "ALTO CARICO ATTIVATO" : "STANDARD RIPRISTINATA", State.peakOverride ? "error" : "info");
};

window.shareApp = async () => {
    if (navigator.share) {
        try {
            await navigator.share({ title: 'Scutum ERP', text: 'Accedi al gestionale', url: window.location.href });
        } catch (err) {}
    } else {
...
window.hf_updateQty = (stateKey, value) => {
    lazzaro_stampMutation(stateKey, 'n_op', Cerbero.cerbero_sanitizeNumber(value).toString());
    window.renderApp();
};

window.hf_updateNote = (stateKey, value) => {
    lazzaro_stampMutation(stateKey, 'note', Cerbero.cerbero_sanitizeText(value));
};
...
window.switchSpaView = (viewId) => {
    ...
    if (target) { target.style.display = viewId === 'app-wrapper' ? 'flex' : 'block'; target.classList.add('active'); }
};
...
function applyRolePermissions() {
    ...
    if(checklistContainer) {
        ...
        if (profile && (profile.linkApertura || profile.linkChiusura)) {
            ...
            if (profile.linkApertura) checklistContainer.innerHTML += `<button class="btn-action solid" style="flex:1; padding:16px; background:var(--success); color:#000; font-weight:800;" onclick="window.open('${profile.linkApertura}', '_blank')"><i class="fa-solid fa-sun"></i> APERTURA</button>`;
            if (profile.linkChiusura) checklistContainer.innerHTML += `<button class="btn-action solid" style="flex:1; padding:16px; background:var(--danger); color:#fff; font-weight:800;" onclick="window.open('${profile.linkChiusura}', '_blank')"><i class="fa-solid fa-moon"></i> CHIUSURA</button>`;
        } else {
            checklistContainer.style.display = 'none';
        }
    }
    const pasteBtn = document.getElementById('floating-paste-btn');
    if (pasteBtn) pasteBtn.style.display = (isAdmin && State.clipboardSection) ? 'flex' : 'none';
}
...
function renderSidebar() {
    ...
    Object.keys(State.appStructure.sedi).forEach(sedeId => {
        ...
        div.onclick = () => {
            State.activeSede = sedeId; State.activeFolder = Object.keys(sede.folders)[0] || null; State.activeFilter = null; window.renderApp();
            if(window.innerWidth <= 768) { const sidebar = document.getElementById('main-sidebar'); if(sidebar) sidebar.classList.remove('open'); }
        };
    ...
    if(filtersMenu) {
        ...
        if (State.activeSede && State.appStructure.sedi[State.activeSede]) {
            ...
            Object.values(State.appStructure.sedi[State.activeSede].folders).forEach(f => { if(f.sections) Object.values(f.sections).forEach(s => catMap.set(s.name, s.color)); });
            catMap.forEach((color, name) => { filtersMenu.innerHTML += `<div class="nav-item ${State.activeFilter === name ? 'active' : ''}" onclick="State.activeFilter='${name}'; window.renderApp();"><span style="display:inline-block; width:12px; height:12px; border-radius:50%; background:${color}; margin-right:12px;"></span> ${name}</div>`; });
        }
    }
}
...
function renderFolders() {
    ...
    Object.entries(State.appStructure.sedi[State.activeSede].folders).forEach(([folderId, folder]) => {
        ...
        let editIconHtml = isAdmin ? `<i class="fa-solid fa-pen" style="margin-left:8px; opacity:0.7;" onclick="event.stopPropagation(); window.editFolder('${folderId}');"></i>` : '';
    ...
    if (isAdmin) foldersMenu.innerHTML += `<button class="folder-tab add" style="border-style:dashed;" onclick="window.openFolderModal()"><i class="fa-solid fa-plus"></i> Nuovo Turno</button>`;
}
...
function renderMainContent() {
    ...
    if (!State.activeSede || !State.activeFolder) { headerTitle.innerText = "SISTEMA VERGINE / NESSUN TURNO"; return; }
    ...
    Object.entries(sections).forEach(([sectionId, section]) => {
        ...
        let itemsToRender = [...(section.items || [])].filter(item => State.peakOverride || !item.days || item.days.length === 0 || item.days.includes(currentDay));
        itemsToRender.sort((a, b) => { if (!a.expiry) return 1; if (!b.expiry) return -1; return new Date(a.expiry) - new Date(b.expiry); });
        ...
        sectionDiv.innerHTML = `<div class="section-header"><h3 style="color:${section.color}; text-transform:uppercase; margin:0;">${section.name}</h3>${adminActions}</div>`;
        ...
        itemsToRender.forEach(item => {
            ...
            if (item.type === 'magazzino') {
                if (item.dailyIdeals && item.dailyIdeals.length === 7) targetIdeal = State.peakOverride ? Math.max(...item.dailyIdeals) : item.dailyIdeals[currentDay];
                else targetIdeal = item.idealQty || 0; 
            }

            let typeBadge = item.type === 'magazzino' ? `<span style="font-size:0.7rem; padding:2px 6px; border-radius:4px; background:#3498db20; color:#3498db;">SOGLIA: ${targetIdeal} ${item.uom || 'pz'}</span>` : '';
            
            let controlsHtml = item.type === 'magazzino' ? `
                <div class="item-controls">
                    <div style="display:flex; align-items:center; gap:6px; background:rgba(0,0,0,0.3); border:1px solid var(--border); border-radius:6px; padding:2px 4px;">
                        <button onclick="window.hf_stepQty('${stateKey}', -1)" style="background:none; border:none; color:var(--accent); font-size:1.2rem; font-weight:800; cursor:pointer;">-</button>
                        <input type="number" class="qty-input" value="${itemState.n_op || ''}" style="width:50px; text-align:center; border:none; background:none; color:var(--text-main); font-weight:700;" onchange="window.hf_updateQty('${stateKey}', this.value)">
                        <button onclick="window.hf_stepQty('${stateKey}', 1)" style="background:none; border:none; color:var(--accent); font-size:1.2rem; font-weight:800; cursor:pointer;">+</button>
                    </div>
                    <span class="unit-label" style="font-weight:700; color:var(--text-muted);">${item.uom || 'pz'}</span>
                </div>` : `
                <div class="item-controls">
                    <div class="input-group-inline"><input type="number" class="qty-input" value="${itemState.n_op || ''}" placeholder="Qt." onchange="window.hf_updateQty('${stateKey}', this.value)"><span class="unit-label">${item.uom || 'pz'}</span></div>
                    <div class="custom-checkbox ${itemState.done ? 'checked' : ''}" onclick="window.toggleDone('${stateKey}')">${itemState.done ? '<i class="fa-solid fa-check"></i>' : ''}</div>
                </div>`;

            const itemDiv = document.createElement('div');
            itemDiv.className = `item-row ${itemState.done ? 'done' : ''}`;
            let itemEditBtn = isAdmin ? `<button style="background:none; border:none; color:var(--accent); margin-left:auto; padding:8px;" onclick="window.hf_editItemModal('${sectionId}', '${item.id}')"><i class="fa-solid fa-pen"></i></button>` : '';

            itemDiv.innerHTML = `
                <div class="item-main">
                    <div class="item-name-group" style="flex:1;">
                        <span class="item-name">${item.name}</span>
                        <div style="display:flex; gap:6px; margin-top:4px;">${typeBadge}</div>
                    </div>
                    ${itemEditBtn}
                    ${controlsHtml}
                </div>
                <div class="item-sub" style="margin-top: 8px;">
                    <input type="text" class="note-input" value="${itemState.note || ''}" placeholder="Aggiungi nota..." onchange="window.hf_updateNote('${stateKey}', this.value)">
                </div>`;
            sectionDiv.appendChild(itemDiv);
        });

        if (isAdmin) sectionDiv.innerHTML += `<button class="btn-action" style="margin:16px; width:calc(100% - 32px); border:1px dashed var(--border);" onclick="window.hf_openItemModal('${sectionId}')"><i class="fa-solid fa-plus"></i> AGGIUNGI PRODOTTO</button>`;
        content.appendChild(sectionDiv);
    });

    if (isAdmin && !State.activeFilter) content.innerHTML += `<button class="btn-action" style="width:100%; margin-top:24px; border:1px dashed var(--text-muted);" onclick="window.openSectionModal()"><i class="fa-solid fa-layer-group"></i> CREA NUOVA CELLA LOGICA</button>`;
}
