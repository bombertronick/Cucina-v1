...
window.openOperatorListModal = () => { 
    // INTERCETTORE BLOCCO SILENTE: Avvisa se non ci sono Sedi
    if (!State.activeSede || !State.appStructure.sedi[State.activeSede]) {
        alert("ATTENZIONE OPERATIVA: Nessuna Sede rilevata.\n\nDevi prima creare una 'Nuova Sede' per poterle assegnare degli operatori.");
        return;
    }
...
window.saveInternalSnapshot = async () => {
    try {
        ...
        if (snapshots.length > 8) snapshots.pop();
...
/**
 * CONTAINER CLOUD MODAL E MACCHINA DEL TEMPO
 */
