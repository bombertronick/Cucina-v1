...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // CONTROLLO SESSIONE PERSISTENTE (Anti-Refresh Volontario o Accidentale)
    ...
    if (activeSession) {
        ...
        window.renderApp();
        return; // Ferma l'esecuzione del bootloader del login, sei già dentro.
    }

    // COSTRUTTORE MODALE LOGIN A MATRIOSKA (ACCORDION REPARTI)
    const profileSelect = document.getElementById('login-profile');
    if (profileSelect) {
        profileSelect.style.display = 'none'; // Nasconde fisicamente la vecchia tendina
        ...
        if (defaultSedeId && State.appStructure.sedi[defaultSedeId]) {
            ...
            if (sede.roles && sede.roles.length > 0) {
                // Algoritmo di Raggruppamento Dinamico per Reparti
                const teams = {};
                ...
                // Generazione della Fisarmonica (Accordion)
                Object.keys(teams).forEach(team => {
                    ...
                    // Generazione degli Operatori interni al Reparto
...
// Variabile globale per tracciare l'utente selezionato
...
window.selectProfileMatryoshka = (profileId, element) => {
    ...
    // Reset di tutte le selezioni visive
    document.querySelectorAll('.matryoshka-op').forEach(el => {
        if (el.innerHTML.includes('ROOT')) {
            el.style.borderColor = 'var(--danger)';
            el.style.background = 'transparent';
        } else {
            el.style.borderColor = 'transparent';
            el.style.background = 'rgba(255,255,255,0.03)';
            el.querySelector('.fa-user').style.color = 'var(--text-muted)';
        }
    });

    // Applicazione dello stile "Attivo/Selezionato"
    if (element.innerHTML.includes('ROOT')) {
        element.style.background = 'rgba(231,76,60,0.1)';
    } else {
        element.style.borderColor = 'var(--accent)';
        element.style.background = 'rgba(201,164,100,0.1)';
        element.querySelector('.fa-user').style.color = 'var(--accent)';
    }
};
